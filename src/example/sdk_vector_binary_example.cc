...
static int32_t g_dimension = 16;
static dingodb::sdk::BinaryFlatParam g_binary_flat_param(g_dimension, dingodb::sdk::MetricType::kHamming);
...
static void PrepareVectorIndex() {
  ...
  Status create = creator->SetSchemaId(g_schema_id)
                      ...
                      .SetBinaryFlatParam(g_binary_flat_param)
...
static void VectorAdd(bool use_index_name = false) {
  ...
  for (const auto& id : g_range_partition_seperator_ids) {
    dingodb::sdk::Vector tmp_vector{dingodb::sdk::ValueType::kUint8, g_dimension};
    tmp_vector.binary_values.push_back(1 + delta);
    tmp_vector.binary_values.push_back(2 + delta);
...
static void VectorSearch(bool use_index_name = false) {
  ...
  for (int i = 0; i < 5; i++) {
    dingodb::sdk::Vector tmp_vector{dingodb::sdk::ValueType::kUint8, g_dimension};
    tmp_vector.binary_values.clear();
    tmp_vector.binary_values.push_back(init);
    tmp_vector.binary_values.push_back(init);
...
static void VectorGetIndexMetrics(bool use_index_name = false) {
  ...
  if (tmp.ok()) {
    CHECK_EQ(result.index_type, dingodb::sdk::VectorIndexType::kBinaryFlat);
