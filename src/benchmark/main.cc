...
void InitLog(const std::string& log_dir) {
  if (!dingodb::benchmark::IsExistPath(log_dir)) {
    dingodb::benchmark::CreateDirectories(log_dir);
  }
...
int main(int argc, char* argv[]) {
  ...
  auto benchmark = dingodb::benchmark::Benchmark::New(environment.GetClientStub(), environment.GetClient());
