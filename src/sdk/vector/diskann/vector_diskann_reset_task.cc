...
// ResetByIndex
Status VectorResetByIndexTask::Init() {
...
void VectorResetByIndexTask::DoAsync() {
  ...
  for (const auto& part_id : next_part_ids) {
    auto* sub_task = new VectorResetPartTask(stub, vector_index_, part_id);
    sub_task->AsyncRun([this, sub_task](auto&& s) { SubTaskCallback(std::forward<decltype(s)>(s), sub_task); });
  }
}

void VectorResetByIndexTask::SubTaskCallback(const Status& status, VectorResetPartTask* sub_task) {
...
// ResetPartTask
void VectorResetPartTask::DoAsync() {
  ...
  for (auto i = 0; i < regions.size(); i++) {
    ...
    controller.AsyncCall(
        [this, rpc = rpcs_[i].get()](auto&& s) { VectorResetRpcCallback(std::forward<decltype(s)>(s), rpc); });
  }
}

void VectorResetPartTask::VectorResetRpcCallback(const Status& status, VectorResetRpc* rpc) {
  if (!status.ok()) {
...
bool VectorResetPartTask::NeedRetry() {
...
// ResetByRegion
Status VectorResetByRegionTask ::Init() {
  std::shared_ptr<VectorIndex> tmp;
  DINGO_RETURN_NOT_OK(stub.GetVectorIndexCache()->GetVectorIndexById(index_id_, tmp));
  DCHECK_NOTNULL(tmp);
  vector_index_ = std::move(tmp);
...
void VectorResetByRegionTask::DoAsync() {
  ...
  if (regions_.empty()) {
    ...
    DoAsyncDone(Status::ResetFailed(""));
  }
  ...
  for (auto i = 0; i < regions_.size(); i++) {
    ...
    controller.AsyncCall(
        [this, rpc = rpcs_[i].get()](auto&& s) { VectorResetByRegionRpcCallback(std::forward<decltype(s)>(s), rpc); });
  }
}

void VectorResetByRegionTask::VectorResetByRegionRpcCallback(const Status& status, VectorResetRpc* rpc) {
  if (!status.ok()) {
    DINGO_LOG(WARNING) << "rpc: " << rpc->Method() << " send to region: " << rpc->Request()->context().region_id()
                       << " fail: " << status.ToString();
    std::unique_lock<std::shared_mutex> w(rw_lock_);
...
bool VectorResetByRegionTask::NeedRetry() {
  for (auto& result : result_.region_status) {
    if (retry_count_ >= FLAGS_vector_op_max_retry) {
      return false;
    }
