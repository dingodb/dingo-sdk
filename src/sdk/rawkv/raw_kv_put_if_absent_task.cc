...
RawKvPutIfAbsentTask::RawKvPutIfAbsentTask(const ClientStub& stub, const std::string& key, const std::string& value,
                                           bool& out_state)
    : RawKvTask(stub), key_(key), value_(value), store_rpc_controller_(stub, rpc_), out_state_(out_state) {}

void RawKvPutIfAbsentTask::DoAsync() {
  ...
  store_rpc_controller_.AsyncCall([this](auto&& s) { KvPutIfAbsentRpcCallback(std::forward<decltype(s)>(s)); });
}

void RawKvPutIfAbsentTask::KvPutIfAbsentRpcCallback(const Status& status) {
  if (status.ok()) {
