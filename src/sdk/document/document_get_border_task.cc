...
Status DocumentGetBorderTask::Init() {
...
void DocumentGetBorderTask::DoAsync() {
  ...
  for (const auto& part_id : next_part_ids) {
    auto* sub_task = new DocumentGetBorderPartTask(stub, vector_index_, part_id, is_max_);
    sub_task->AsyncRun([this, sub_task](auto&& s) { SubTaskCallback(std::forward<decltype(s)>(s), sub_task); });
  }
}

void DocumentGetBorderTask::SubTaskCallback(Status status, DocumentGetBorderPartTask* sub_task) {
...
void DocumentGetBorderPartTask::DoAsync() {
  ...
  for (auto i = 0; i < regions.size(); i++) {
    ...
    controller.AsyncCall(
        [this, rpc = rpcs_[i].get()](auto&& s) { DocumentGetBorderIdRpcCallback(std::forward<decltype(s)>(s), rpc); });
  }
}

void DocumentGetBorderPartTask::DocumentGetBorderIdRpcCallback(const Status& status, DocumentGetBorderIdRpc* rpc) {
  if (!status.ok()) {
