...
#ifndef DINGODB_SDK_DOCUMENT_UPDATE_TASK_H_
#define DINGODB_SDK_DOCUMENT_UPDATE_TASK_H_
...
class DocumentUpdateTask : public DocumentTask {
 public:
  DocumentUpdateTask(const ClientStub& stub, int64_t index_id, std::vector<DocWithId>& docs)
      : DocumentTask(stub), index_id_(index_id), docs_(docs) {}

  ~DocumentUpdateTask() override = default;

 private:
  Status Init() override;
  void DoAsync() override;

  std::string Name() const override { return fmt::format("DocumentUpdateTask-{}", index_id_); }

  void DocumentAddRpcCallback(const Status& status, DocumentAddRpc* rpc);

  const int64_t index_id_;
  const std::vector<DocWithId>& docs_;
