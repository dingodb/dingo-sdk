...
void TxnCheckSecondaryLocksTask::TxnCheckSecondaryLocksRpcCallback(const Status& status,
  ...
  DINGO_LOG(DEBUG) << fmt::format("[sdk.txn.{}] rpc: {} request: {} response: {}", primary_lock_start_ts_, rpc->Method(),
                                  rpc->Request()->ShortDebugString(), rpc->Response()->ShortDebugString());
  Status s;
  const auto* response = rpc->Response();
  if (!status.ok()) {
    DINGO_LOG(WARNING) << fmt::format("[sdk.txn.{}] rpc: {} send to region: {} fail: {}", primary_lock_start_ts_, rpc->Method(),
    ...
    if (response->has_txn_result()) {
      ...
      if (s1.IsTxnNotFound()) {
        WriteLockGuard guard(rw_lock_);
        txn_check_secondary_status_.not_found_keys.push_back(response->txn_result().txn_not_found().primary_key());
      } else if (!s1.ok()) {
        s = s1;
        DINGO_LOG(WARNING) << fmt::format("[sdk.txn.{}] txn check secondary keys fail, status({}) , txn_result({}).",
                                          primary_lock_start_ts_, s.ToString(), response->txn_result().ShortDebugString());
      }
    }
    ...
    if (!response->has_txn_result() && response->locks_size() == 0 && response->commit_ts() == 0) {
      WriteLockGuard guard(rw_lock_);
