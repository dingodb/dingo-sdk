...
Status TxnImpl::PreCommit1PC() {
  ...
  TxnPrewriteTask task(stub_, buffer_->GetPrimaryKey(), mutations_map, shared_from_this(), is_one_pc, use_async_commit,
                       mutations_map_primary_key, min_commit_ts);
...
Status TxnImpl::PreCommit2PC() {
  ...
  TxnPrewriteTask task_primary(stub_, buffer_->GetPrimaryKey(), mutations_map_primary_key, shared_from_this(),
                               is_one_pc, use_async_commit, mutations_map_ordinary_keys, min_commit_ts);
