...
Status TxnLockResolver::ResolveLockSecondaryLocks(const pb::store::LockInfo& primary_lock_info, int64_t start_ts,
  ...
  TxnCheckSecondaryLocksTask txn_check_secondary_locks_task(stub_, std::move(secondary_keys), primary_lock_info.lock_ts(),
                                                            txn_secondary_lock_status);
  Status status = txn_check_secondary_locks_task.Run();
  if (!status.ok()) {
    DINGO_LOG(WARNING) << fmt::format(
        "[sdk.txn.{}] check secondary locks fail, primary_lock_info({}), txn_status({}), status({}).", primary_lock_info.lock_ts(),
        primary_lock_info.ShortDebugString(), txn_status.ToString(), status.ToString());
    return status;
  }
  CHECK(!txn_secondary_lock_status.IsError()) << fmt::format("[sdk.txn.{}]unexpected txn_secondary_lock_status: {}",
                                                             primary_lock_info.lock_ts(), txn_secondary_lock_status.ToString());

  if (txn_secondary_lock_status.is_rollbacked || txn_secondary_lock_status.not_found_keys.size() > 0) {
    // rollback all keys
    ...
    for (auto& lock : txn_secondary_lock_status.locked_keys) {
      ...
      CHECK(lock.lock_ts() == primary_lock_info.lock_ts())
          << fmt::format("[sdk.txn.{}] inconsistent lock_ts, primary_lock_info({}) lock({}) txn_secondary_lock_status({}).", start_ts,
