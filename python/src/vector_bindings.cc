...
void DefineVectorBindings(pybind11::module& m) {
  ...
  py::class_<DiskAnnParam>(m, "DiskAnnParam").def(py::init<>());  // 定义一个无参数的构造函数
  ...
  py::class_<VectorIndexCreator>(m, "VectorIndexCreator")
      // .def(py::init<>())
      .def("SetSchemaId", &VectorIndexCreator::SetSchemaId)  // 需不需要添加py::return_value_policy::reference
  ...
  py::class_<VectorClient>(m, "VectorClient")
      // .def(py::init<>())
      .def("AddByIndexId", &VectorClient::AddByIndexId)
      .def("AddByIndexName", &VectorClient::AddByIndexName)
      .def("SearchByIndexId",
